...
const data = require("./dec.geo.json");
...
const mapElement = document.querySelector("leaflet-map");
  ...
  orangeBlueArray = ['#F4BD6E', '#ED8A52', '#E34D32', '#B22118'];

  // ['#bfe0f5', 'gray', '#F4BD6E', '#ED8A52', '#E34D32', '#B22118'];

  var arrayLegend = {
    dec3_point_change_array: [5.0, 10.0, 15.0],
    dec3_per_pos_clean_array: [10.0,15.0,20.0,25.0],
    pos_change: "dec3_point_change",
    tests_pos: "dec3_per_pos_clean"
  };
  ...
  data.features.forEach(function(f) {
    ...
    // });
    ["dec3_tested", "dec3_Positives", "dec3_Hospitalizations"].forEach(function(prop) {
  ...
  var getColor = function(d, array) {
    ...
    let chosenColorArray = (array === "dec3_per_pos_clean_array") ? orangeArray : orangeBlueArray;

    array === "dec3_point_change_array" ? lastBlocks.forEach(el => el.style.display = 'none') : lastBlocks.forEach(el => el.style.display = 'block');
    // array === "dec3_per_pos_clean_array" ? pos_neg.forEach(el => el.classList.add('hide')) : pos_neg.forEach(el => el.classList.remove('hide'));
